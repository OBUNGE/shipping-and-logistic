...
module TikTokEvents
  ...
  def self.track_view_content(product, user)
    track_event(
      ...
      properties: {
        value: product.price.to_f,          # ✅ numeric only
        currency: "KES",
        content_id: product.sku,            # ✅ SKU/catalog ID
  ...
  def self.track_add_to_cart(cart_item, user)
    track_event(
      ...
      event_time: Time.now.to_i,
      properties: {
        value: cart_item.price.to_f,        # ✅ numeric only
        currency: "KES",
        content_id: cart_item.product.sku,  # ✅ SKU/catalog ID
        content_type: "product",
  ...
  def self.track_checkout(order, user)
    track_event(
      ...
      event_time: Time.now.to_i,
      properties: {
        value: order.total_price.to_f,              # ✅ numeric only
        currency: "KES",
